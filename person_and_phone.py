...
def main(video_path):
    ...
    while(True):
        ...
        for i in range(nums[0]):
            ...
            if int(classes[0][i] == 67):
                ...
                cv2.imwrite(os.path.join('results',os.path.splitext(os.path.basename(video_path))[0] + '.png'), img)
                results_list.append('Mobile Phone detected')
                return img,list(set(results_list))
                # break

        if count == 0:
            print('No person detected')
            cv2.imwrite(os.path.join('results',os.path.splitext(os.path.basename(video_path))[0] + '.png'), img)
            results_list.append('No person detected')

        elif count > 1:
            print('More than one person detected')
            cv2.imwrite(os.path.join('results',os.path.splitext(os.path.basename(video_path))[0] + '.png'), img)
            results_list.append('More than one person detected')
    ...
    return None,list(set(results_list))